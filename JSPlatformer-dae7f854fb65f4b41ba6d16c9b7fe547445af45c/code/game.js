...
function Square(pos){
  this.basePos = this.pos = pos.plus(new Vector(0.1,0.0));
  this.size = new Vector(1.0, 1.0);
  this.wobbling = Math.random() * Math.PI * 4;
}
...
Level.prototype.obstacleAt = function (pos, size) {
  ...
  // Consider the sides and top and bottom of levels as walls
...
var wobblingSpeed = 20;
var wobblingDist = .14;
Square.prototype.act = function(step) {
  this.wobbling += step * wobblingSpeed;
  var wobblingPos = Math.sin(this.wobbling) * wobblingDist;
  this.pos = this.basePos.plus(new Vector (wobblingPos, -0.5));
};
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  // The floor is also an obstacle -- only allow players to jumps
